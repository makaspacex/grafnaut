...
export const strongPasswordValidations: StrongPasswordValidation[] = [
  {
    messageKey: 'core.validation-labels.strong-password-validations.message-at-least-12-characters',
    defaultMessage: 'At least 12 characters',
    validation: (value: string) => value.length >= 12,
  },
  {
    messageKey: 'core.validation-labels.strong-password-validations.message-one-uppercase-letter',
    defaultMessage: 'One uppercase letter',
    validation: (value: string) => /[A-Z]+/.test(value),
  },
  {
    messageKey: 'core.validation-labels.strong-password-validations.message-one-lowercase-letter',
    defaultMessage: 'One lowercase letter',
    validation: (value: string) => /[a-z]+/.test(value),
  },
  {
    messageKey: 'core.validation-labels.strong-password-validations.message-one-number',
    defaultMessage: 'One number',
    validation: (value: string) => /[0-9]+/.test(value),
  },
  {
    messageKey: 'core.validation-labels.strong-password-validations.message-one-symbol',
    defaultMessage: 'One symbol',
...
export const ValidationLabel = ({ strongPasswordValidation, password, pristine }: ValidationLabelProps) => {
  ...
  const { messageKey, defaultMessage, validation } = strongPasswordValidation;
